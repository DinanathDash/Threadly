/**
 * Server Logger utility for Threadly application
 * 
...
// Set this to false to disable all non-critical logging in production
const LOGGING_ENABLED = process.env.NODE_ENV === 'development' || process.env.SERVER_LOGGING === 'true';

// Set to true to show minimal logs even in production (for critical errors only)
const ENABLE_CRITICAL_LOGS = true;

/**
 * Safe logging function that only logs in development mode or when explicitly enabled
...
/**
 * Log warning messages - only shown in development or when explicitly enabled
...
/**
 * Log info messages - only shown in development or when explicitly enabled
